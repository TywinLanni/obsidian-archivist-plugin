...
- [ ] **Calendar field в categories.md** — парсить колонку `Calendar` из `categories.md` (значения: `google` или пусто). Включать поле `calendar` в `PUT /v1/categories`. При `GET /v1/categories` — читать `calendar` и записывать обратно в таблицу `categories.md`. Формат таблицы: `| Category | Description | Reminder | Calendar |`.
