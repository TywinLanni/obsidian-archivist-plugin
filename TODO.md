...
- [x] **Calendar field в categories.md** — парсить колонку `Calendar` из `categories.md` (значения: `google` или пусто). Включать поле `calendar` в `PUT /v1/categories`. При `GET /v1/categories` — читать `calendar` и записывать обратно в таблицу `categories.md`. Формат таблицы: `| Category | Description | Reminder | Calendar |`.
  Bot implementation: `core/src/archivistbot_core/storage/models.py` (Category.calendar), `core/openapi.yaml` (CategoryItem.calendar).
  API: `PUT /v1/categories` accepts `calendar` field, `GET /v1/categories` returns it (see `commercial/src/archivistbot_commercial/api_server.py`).
